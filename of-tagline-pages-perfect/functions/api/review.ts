...
function widenWalkingMinutes(text: string) {
  return text.replace(/徒歩(\d{1,2})分(?![~〜]\d)/g, (_m, p1) => {
    const base = Number(p1); if (!Number.isFinite(base)) return _m;
...
/* ---------- 長さ調整 & 校正 ---------- */
async function ensureLengthDescribe(apiKey: string, opts: { draft: string; min: number; max: number; tone: string; style: string; }) {
  let out = opts.draft || "";
  for (let i = 0; i < 3; i++) {
...
export const onRequestPost: PagesFunction = async (ctx) => {
  try {
    ...
    // 1) 元文クリーニング（価格表現 → 規約適合）
    let cleaned = stripPriceAndSpaces(String(text));
    cleaned = enforceKiyaku(cleaned);

    // 2) 長さ調整 → 3) 校正 → 4) 最終クリーニング
    cleaned = await ensureLengthDescribe(OPENAI_API_KEY, { draft: cleaned, min: minChars, max: maxChars, tone, style: STYLE });
    cleaned = await polishJapanese(OPENAI_API_KEY, cleaned, tone, STYLE);
